...
def build_graph_MSCT_asyn_v1(CSI, dist, delays, etas, norm_csi_real, norm_csi_imag, K, threshold):
    ...
    for i in range(n):
        for j in range(n):
            ...
            # else:
            #     dist2[i][j] = 1
            if ((i % users) != (j % users)) and (i / users == j / users):
                dist2[i][j] = 1
            # if i == j:
            #     dist2[i][j] = 0
            # elif (i % users) == (j % users):
...
class IGConv(MessagePassing):
    ...
    def update(self, aggr_out, x):
        ...
        # if (square_sum <= 1).item():
        #     comb_normalized = comb_group / torch.sqrt(square_sum)
        # else:
        #     comb_normalized = comb_group
    ...
    for iter in range(H6.shape[0]):
        ...
        for s in range(train_S):
            ...
            if mode_square_sum > 1:
                W_k[:,s*Nt:(s+1)*Nt] = torch.sqrt(torch.tensor(train_S/users)) * submatrix / torch.sqrt(mode_square_sum)
        p_update1 = get_p_from_W(W_k, p3[iter, :, :, :])
        rx_all_power1 = torch.mul(H7[iter, :, :, :], p_update1)
        ...
        for s in range(train_S):
            ...
            if mode_square_sum_mmse > 1:
                W_k_mmse[:,s*Nt:(s+1)*Nt] = torch.sqrt(torch.tensor(train_S/users)) * submatrix_mmse / torch.sqrt(mode_square_sum_mmse)
        p_update_mmse = get_p_from_W(W_k_mmse, p3[iter, :, :, :])
        rx_all_power_2 = torch.mul(H7[iter, :, :, :], p_update_mmse)
        rx_all_power_2 = torch.sum(rx_all_power_2, axis=-1)
        for j in range(users):
            _, _, _, SINR_k_mmse = calculate_SINR(
                iter, rx_all_power_2, initial_delay, add_delta_delay, j, noise_powers[j], eta_enable, eta_add)
            rate_MMSE[iter] += np.log2(1 + SINR_k_mmse.item())

        ## async ZF
        for s in range(train_S):
            submatrix_zf = W_k_mmse[:,s*Nt:(s+1)*Nt]
            mode_square_sum_zf = torch.sum(torch.abs(submatrix_zf)**2)
            if mode_square_sum_zf > 1:
                W_k_zf[:,s*Nt:(s+1)*Nt] = torch.sqrt(torch.tensor(train_S/users)) * submatrix_zf / torch.sqrt(mode_square_sum_zf)
        p_update_zf = get_p_from_W(W_k_zf, p3[iter, :, :, :])
        rx_all_power_3 = torch.mul(H7[iter, :, :, :], p_update_zf)
        rx_all_power_3 = torch.sum(rx_all_power_3, axis=-1)
        for j in range(users):
            _, _, _, SINR_k_zf = calculate_SINR(
                iter, rx_all_power_3, initial_delay, add_delta_delay, j, noise_powers[j], eta_enable, eta_add)
...
def sr_loss_all_test(data, p, K, N, epoch, imperfect_channel, add_mode):
    ...
    # print('sr_loss===reshape p2:{}, size:{}'.format(p2, p2.shape))
    p3 = torch.complex(p1, p2)
    # p_all = torch.complex(p1, p2)
    # p3 = p_all.detach()
    print('sr_loss===complex p3:{}, size:{}'.format(p3, p3.shape))
    # for i in range(p3.shape[0]):
    #     for j in range(0, p3.shape[1], users):
    #         submat = p3[i, j:j+users, :, :]
    #         mode_square_sum = torch.sum(torch.abs(submat) ** 2)
    #         if mode_square_sum > 1:
    #             p3[i, j:j + users, :, :] = submat / torch.sqrt(mode_square_sum)
    ...
    if only_GNN == 0 and (epoch == 1 or (epoch == Epoches-1)):
        ...
        print('sr_loss===start to compute async_WMMSE and sync_WMMSE')
        asyn_wmmse_rate, asyn_mmse_rate, asyn_zf_rate = compute_asyn_wmmse(H3, H_new, p3, initial_delay, add_delta_delay, 1, 1)
        asyn_wmmse_rate_noadd, asyn_mmse_rate_noadd, asyn_zf_rate_noadd = compute_asyn_wmmse(H3, H_new, p3, initial_delay, add_delta_delay, 1, 0)
        syn_wmmse_rate, syn_mmse_rate, syn_zf_rate = compute_asyn_wmmse(H3, H_new, p3, initial_delay, add_delta_delay, 0, 0)
        end_wmmse = time.time()
    ...
    for iter in range(rx_all_power.shape[0]):
        ...
        for user_index in range(users):
            ...
            valid_power = torch.empty(0).cuda()
            interference_sum_power_syn = torch.empty(0).cuda()
            interference_sum_power_asyn_no_add = torch.empty(0).cuda()
            interference_sum_power_asyn_add = torch.empty(0).cuda()
            for link in range(rx_all_power.shape[1]):
                ...
                if int(link % users) == user_index:
                    valid_signal = torch.cat(
                        (valid_signal, rx_all_power[iter, link, link].view(1)))
                    transmit_power += torch.sum(p3[iter, link, 0, :].abs() ** 2)
                # else:
                #     valid_link = satellite_index*users+user_index
                #     interference_signal = torch.cat(
                #         (interference_signal, rx_all_power[iter, link, user_index].view(1)))
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        optimizer.step()
    print('No-Cooperative SNR {:03d}dB, IPC {:03d},  Epoch {:03d}, Train Loss: {:.4f}, avr_rate_syn: {:.4f}, avr_rate_asyn_no_add: {:.4f}, '
    ...
    print('No-Cooperative SNR {:03d}dB, IPC {:03d},  Epoch {:03d}, Train Loss: {:.4f}, avr_rate_syn_imperfect: {:.4f}, avr_rate_asyn_no_add_imperfect: {:.4f}, '
