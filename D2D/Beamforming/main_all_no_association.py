...
def compute_asyn_wmmse(H3, p3, initial_delay, add_delta_delay, eta_enable, eta_add):
    H6 = torch.clone(H3)
    H7 = H3.conj()
...
def sr_loss_all(data, p, K, N, epoch):
    # H1 K*K*N
    # p1 K*N
    eta_enable = 1
    eta_add = 1
...
def sr_loss_all_test(data, p, K, N, epoch):
    # H1 K*K*N
    # p1 K*N
    eta_enable = 1
    eta_add = 1
    ...
    if epoch == 1:
        print('sr_loss===start to compute async_WMMSE and sync_WMMSE')
        asyn_wmmse_rate = compute_asyn_wmmse(H3, p3, initial_delay, add_delta_delay, 1, 1)
        asyn_wmmse_rate_noadd = compute_asyn_wmmse(H3, p3, initial_delay, add_delta_delay, 1, 0)
        syn_wmmse_rate = compute_asyn_wmmse(H3, p3, initial_delay, add_delta_delay, 0, 0)
        print('sr_loss===end to compute async_WMMSE and sync_WMMSE')
    # rx_all_power_1 = torch.abs(rx_all_power)**2
    rate_iter_syn = torch.empty(0).cuda()
    ...
    loss = torch.neg(avr_rate_asyn_add)
    print('sr_loss===loss:{}'.format(loss))
    return loss, avr_rate_syn, avr_rate_asyn_no_add, avr_rate_asyn_add, syn_wmmse_rate, asyn_wmmse_rate_noadd, asyn_wmmse_rate
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        loss, avr_rate_syn, avr_rate_asyn_no_add, avr_rate_asyn_add, sync_wmmse_rate, asyn_wmmse_rate_noadd, async_wmmse_rate = sr_loss_all_test(data, out, train_K, Nt, epoch)
        loss.backward()
...
def test(epoch):
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
            ...
            loss, _, _, _, _, async_wmmse_rate = sr_loss_all(data, out, test_K, Nt, epoch)
...
for epoch in range(1, 500):
    ...
    print('SNR {:03d}dB, Epoch {:03d}, Train Loss: {:.4f}, avr_rate_syn: {:.4f}, avr_rate_asyn_no_add: {:.4f}, '
          ...
          'Average asyncWMMSE Rate: {:.4f}'.format(SNR_dB, epoch, loss1, avr_rate_syn, avr_rate_asyn_no_add,
