...
def np_WMMSE_vector(b_int, H, Pmax, var_noise):
    ...
    print('np_WMMSE_vector====H:{}, size:{}'.format(H, H.shape))
    print('np_WMMSE_vector====f:{}, size:{}'.format(f, f.shape))
    print('np_WMMSE_vector====w:{}, size:{}'.format(w, w.shape))
...
def IC_sum_rate(H,p, var_noise):
    # H1 K*K*N
    # p1 K*N
    print('IC_sum_rate====H:{}, size:{}'.format(H, H.shape))
    print('IC_sum_rate====p:{}, size:{}'.format(p, p.shape))
    K = H.shape[1]
    N = H.shape[-1]
    print('IC_sum_rate====K:{}, N:{}'.format(K, N))
    p = p.reshape((-1,K,1,N))
    print('IC_sum_rate====reshape p:{}, size:{}'.format(p, p.shape))
    rx_power = np.multiply(H, p)
    print('IC_sum_rate====rx_power:{}, size:{}'.format(rx_power, rx_power.shape))
    rx_power = np.sum(rx_power,axis=-1)
    print('IC_sum_rate====sum rx_power:{}, size:{}'.format(rx_power, rx_power.shape))
    rx_power = np.square(np.abs(rx_power))
    print('IC_sum_rate====square rx_power:{}, size:{}'.format(rx_power, rx_power.shape))
    mask = np.eye(K)
    valid_rx_power = np.sum(np.multiply(rx_power, mask), axis=1)
    print('IC_sum_rate====valid_rx_power:{}, size:{}'.format(valid_rx_power, valid_rx_power.shape))
    interference = np.sum(np.multiply(rx_power, 1 - mask), axis=1) + var_noise
    print('IC_sum_rate====interference:{}, size:{}'.format(interference, interference.shape))
    rate = np.log2(1 + np.divide(valid_rx_power, interference))
    print('IC_sum_rate====rate:{}, size:{}'.format(rate, rate.shape))
    sum_rate = np.mean(np.sum(rate, axis=1))
    print('IC_sum_rate====sum_rate:{}, size:{}'.format(sum_rate, sum_rate.shape))
    return sum_rate
