...
from torch.nn import Sequential as Seq, Linear as Lin, ReLU, LayerNorm, Dropout, Sigmoid, BatchNorm1d as BN
...
def build_graph_MSCT_asyn_v1(CSI, dist, delays, etas, norm_csi_real, norm_csi_imag, K, threshold):
    ...
    # norm_csi_imag_max = norm_csi_imag.std(axis=(0, 1), keepdims=True)  # 计算每个天线特征的最大值
    # # Min-Max 标准化
    # # norm_csi_real = (norm_csi_real - norm_csi_real_min) / (norm_csi_real_max - norm_csi_real_min)
    # # norm_csi_imag = (norm_csi_imag - norm_csi_imag_min) / (norm_csi_imag_max - norm_csi_imag_min)
    # norm_csi_real = (norm_csi_real - norm_csi_real_min) / norm_csi_real_max
    # norm_csi_imag = (norm_csi_imag - norm_csi_imag_min) / norm_csi_imag_max
    edge_attr_real = norm_csi_real[attr_ind]
    edge_attr_imag = norm_csi_imag[attr_ind]
    # edge_attr_real = norm_csi_real_edge[attr_ind]
    # edge_attr_imag = norm_csi_imag_edge[attr_ind]
    delta_delay = delays[attr_ind]
    delta_delay = np.expand_dims(delta_delay, axis=-1)
    print('delta_delay reshape:{}'.format(delta_delay))
    eta = etas[attr_ind]
...
class IGConv(MessagePassing):
    def __init__(self, mlp1, mlp2, node_feat_dim, edge_feat_dim, heads=2, leaky_slope=0.01, att_dropout=0.01,
        ...
        att_in_dim = self.node_feat_dim + self.node_feat_dim + self.edge_feat_dim
        # att_in_dim = self.node_feat_dim + self.edge_feat_dim
        att_in_dim = self.node_feat_dim + self.node_feat_dim
        att_in_dim = self.node_feat_dim
        self.edge_mlp = nn.Sequential(nn.Linear(self.edge_feat_dim, 64), nn.ReLU(), nn.Linear(64, heads))
        # self.att_lin = nn.Linear(att_in_dim, heads, bias=False)
        # self.att_lin = Lin(mlp1[-1][-2].out_features * 2 + 1, heads, bias=False)
        self.att_lin = Lin(att_in_dim, heads, bias=False)
        self.beta = nn.Parameter(torch.tensor(0.1))
        # Normalization layers
        self.norm = LayerNorm(1 + 2 * Nt + (2 * Nt - 1))  # match feature dim after update
        self.feat_dropout_layer = Dropout(feat_dropout)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        print('unsqueeze x:{}, size:{}'.format(x, x.shape))
        edge_attr = edge_attr.unsqueeze(-1) if edge_attr.dim() == 1 else edge_attr
        ### Norm
        # x = self.norm(x)
        out = self.propagate(edge_index, x=x, edge_attr=edge_attr)
        print('forward====out:{}, size:{}'.format(out, out.shape))
        # if x.shape[1] == out.shape[1]:  # only if dims match
        #     out = out + x
        # out = self.norm(out)
        # out = self.feat_dropout_layer(out)
        return out
        # return self.propagate(edge_index, x=x, edge_attr=edge_attr)

    def message(self, x_i, x_j, edge_attr, index, ptr=None, size_i=None):
...
def sr_loss_all_test(data, p, K, N, epoch, imperfect_channel, add_mode):
    ...
    add_delta_delay = torch.sigmoid(add_delta_delay/0.2) #
    print('sr_loss===original add_delta_delay: {} sizez:{}'.format(add_delta_delay, add_delta_delay.shape))
    ...
    initial_delay = 0.5*torch.reshape(initial_delay, (-1, K, K, 1))
    ...
    change_add_delay = 100*(change_add_delta_delay.unsqueeze(2)-change_add_delta_delay.unsqueeze(1))
