    ...
    for i in range(dist2.shape[0]):
        for j in range(dist2.shape[1]):
            ...
            if i == j:
                ...
                dist2[i][j] = 1
            # if ((i/users) == (i%users)) and  ((j/users) == (j%users)):
            #     if i == j:
            #         dist2[i][j] = 0
            #     else:
            #         dist2[i][j] = 1
...
def sr_loss_asyn(data, p, K, N):
    ...
    for iter in range(rx_all_power.shape[0]):
        ...
        for user_index in range(users):
            ...
            print('sr_loss===user: {} single_connection rate:{}, size:{}'.format(user_index, rate, rate.shape))
            ## 计算WMMSE
        ...
        print('sr_loss===iter: {} single_connection rate:{}, size:{}'.format(iter, rate_iter, rate_iter.shape))
    avr_rate = torch.mean(rate_iter)
    print('sr_loss===single_connection average rate: {}, size:{}'.format(avr_rate, avr_rate.shape))
...
def train():
    ...
    for data in train_loader:
        ...
        total_wmmse_rate += wmmse_rate * data.num_graphs
        print('train====data.num_graphs:{}'.format(data.num_graphs))
        optimizer.step()
    return total_loss / train_layouts, total_wmmse_rate / train_layouts
...
for epoch in range(1, 500):
    ...
    loss1, avr_wmmse_rate = train()
    print('Start to test...')
    loss2, average_wmmse = test()
    print('Epoch {:03d}, Train Loss: {:.4f}, Train average WMMSE Rate: {:.4f}, Val Loss: {:.4f}, Average WMMSE Rate: {}'.format(
        epoch, loss1, avr_wmmse_rate, loss2, average_wmmse))
