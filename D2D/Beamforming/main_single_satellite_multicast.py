...
class IGConv(MessagePassing):
    ...
    def update(self, aggr_out, x):
        ...
        # comb_group_varify = comb.view(-1,users,2*Nt)
        # square_sum_varify = (comb_group_varify ** 2).sum(dim=(1,2), keepdim=True)
        # print('update===varify square_sum_varify comb:{}'.format(square_sum_varify))
...
def compute_asyn_wmmse(H3, p3, initial_delay, add_delta_delay):
    H4 = H3.conj()
    ...
    H = torch.clone(H3)
    another_H = torch.zeros(H.shape[0], users, train_S*H.shape[-1])
    for iter in range(H.shape[0]):
        a_k = torch.zeros(users)
        u_k = torch.zeros(users)
        v_k = torch.ones(users) * 0.1
        H_k = torch.zeros(users, train_S*Nt, train_S*Nt)
        for user_index in range(users):
            for satellite_index in range(train_S):
                start_index = satellite_index*H.shape[-1]
                end_index = (satellite_index+1)*H.shape[-1]
                another_H[iter, user_index, start_index:end_index] = H[iter, satellite_index*users+user_index,:]
                for another_satellite_index in range(train_S):
                    for another_user in range(users):
                        Delta_tau_i = initial_delay[satellite_index*users+another_user, satellite_index*users+user_index].view(1)
                        Delta_tau_j = initial_delay[another_satellite_index*users+another_user, another_satellite_index*users+user_index].view(1)
                        eta_value = calculate_eta(Delta_tau_i, Delta_tau_j)
                        H_k[user_index, satellite_index*Nt:satellite_index*Nt+Nt, another_satellite_index*Nt:another_satellite_index*Nt+Nt, :, :] += eta_value * \
                                                                               H[iter, satellite_index*users+another_user, satellite_index*users+another_user,:] * \
                                                                               H[iter, another_satellite_index*users+another_user, another_satellite_index * users + another_user, :].conj()
         # = torch.zeros(H.shape[0], users, train_S*H.shape[-1])
        W = 1 / np.sqrt(Nt*train_S) * np.ones((users, train_S*H.shape[-1]), dtype=complex)
        t = 0
        t_max = 100
        sinrs = torch.empty(0).cuda()
        while t < t_max:
            t += 1
            p_update = get_p_from_W(W, p3[iter,:,:,:])
            rx_all_power = torch.mul(H4, p_update)
            rx_all_power = torch.sum(rx_all_power, axis=-1)
            for u_index in range(users):
                _, inference, sinr_k = calculate_SINR(rx_all_power, initial_delay, add_delta_delay, iter, u_index)
                a_k[u_index] = torch.sum(torch.mul(W[u_index,:].conj(), another_H[iter, u_index])) * (1/inference)
                u_k[u_index] = 1 + sinr_k
                I_k = torch.eye(H_k[u_index].shape)
                v_k[u_index] = u_k[u_index] * torch.abs(a_k[u_index]) ** 2
                tmp = u_k[u_index] * torch.abs(a_k[u_index])**2 * H_k + v_k[u_index] * I_k
                W[u_index, :] = torch.linalg.inv(tmp) * another_H[iter, u_index] * a_k[u_index].conj() * u_k[u_index]
...
def get_p_from_W(W, p3):
    p_update = torch.zeros_like(p3)
    for i in range(p3.shape[0]):
        user_index = i % users
        satellite_index = i / users
        p_update[i, 0, :] = W[user_index, satellite_index*Nt:(satellite_index+1)*Nt]
    return p_update

def calculate_SINR(rx_all_power, initial_delay, add_delta_delay, iter, user_index):
    # H4 = H3.conj()
    # print('sr_loss===H4:{}, size:{}'.format(H4, H4.shape))
    # rx_all_power = torch.mul(H4, p3)
    eta_enable = 1
    ...
    for link in range(rx_all_power.shape[0]):
        ...
        if int(link % users) == user_index:
            valid_signal = torch.cat(
                (valid_signal, rx_all_power[link, user_index].view(1)))
        # else:
        ...
        if other_user_index != user_index:
            for other_link in range(rx_all_power.shape[0]):
                if int(other_link % users) == other_user_index:
                    ...
                    valid_link = satellite_index * users + user_index
                    interference_signal = torch.cat(
                        (interference_signal, rx_all_power[ other_link, user_index].view(1)))
                    Delta_tau = torch.cat(
            ...
            print(
                'sr_loss===valid_signal: {} interference_signal:{} interference_signal_conj:{} Delta_tau:{}'.format(
            ...
            if eta_enable == 0:
                ...
                interference_sum_power = torch.cat((interference_sum_power, interference_power))
            else:
                interference_power = torch.outer(interference_signal, interference_signal_conj)
                print('sr_loss===outer interference_power: {} size:{}'.format(interference_power,
                ...
                print('sr_loss===mul interference_power: {} size:{}'.format(interference_power,
                                                                            interference_power.shape))
                interference_power = torch.sum(interference_power)
                print(
                    'sr_loss===interference_power:{} size:{}'.format(interference_power, interference_power.shape))
    ...
    interference_sum_power = torch.sum(torch.abs(interference_sum_power)) + 0
    ...
    print('sr_loss===valid_power: {} interference_power:{}'.format(valid_power, interference_sum_power))
    sinr = torch.div(torch.abs(valid_power), torch.abs(interference_sum_power))
    # rate += torch.log2(1 + torch.div(torch.abs(valid_power), torch.abs(interference_sum_power)))
    print('sr_loss===user: {} sinr:{}'.format(user_index, sinr))
    return valid_power, interference_sum_power, sinr
...
def sr_loss_asyn(data, p, K, N):
    ...
    print('sr_loss===start to compute async_WMMSE')
    compute_asyn_wmmse(H3, p3, initial_delay, add_delta_delay)
                        ...
                        for i in range(Delta_tau.shape[0]):
                            for j in range(Delta_tau.shape[0]):
                                if eta_add == 0:
                                    ...
                                    eta_value[i, j] = calculate_eta(Delta_tau[i]+Add_delta_tau[i], Delta_tau[j]+Add_delta_tau[j])
                        print('sr_loss===config eta_value: {} sizez:{}'.format(eta_value, eta_value.shape))
                        interference_power = torch.mul(interference_power, eta_value)
...
def sr_loss_all(data, p, K, N):
    ...
    for iter in range(rx_all_power.shape[0]):
        ...
        for user_index in range(users):
            ...
            for other_user_index in range(users):
                ...
                if other_user_index != user_index:
                    ...
                    print('sr_loss===interference_sum_power_syn:{} interference_sum_power_asyn_no_add: {} interference_sum_power_asyn_add:{}'.format(
                        ...
                        # interference_power = torch.sum(interference_signal_outer_async)
            interference_sum_power_syn = torch.sum(interference_sum_power_syn) + noise_power
            interference_sum_power_asyn_no_add = torch.sum(interference_sum_power_asyn_no_add) + noise_power
            interference_sum_power_asyn_add = torch.sum(interference_sum_power_asyn_add) + noise_power
            # print('sr_loss===Delta_tau: {} sizez:{}'.format(Delta_tau, Delta_tau.shape))
            valid_signal_conj = valid_signal.conj()
            valid_power = torch.sum(torch.outer(valid_signal, valid_signal_conj))
            ...
            rate_syn += torch.log2(1 + torch.div(torch.abs(valid_power), torch.abs(interference_sum_power_syn)))
            rate_asyn_no_add += torch.log2(1 + torch.div(torch.abs(valid_power), torch.abs(interference_sum_power_asyn_no_add)))
            rate_asyn_add += torch.log2(1 + torch.div(torch.abs(valid_power), torch.abs(interference_sum_power_asyn_add)))
    ...
    sr = wmmse.IC_sum_rate(CSI.numpy(), Y, var)
    print('WMMSE rate at sr_loss:', sr)
    return loss, avr_rate_syn, avr_rate_asyn_no_add, avr_rate_asyn_add, sr
...
def train():
    ...
    for data in train_loader:
        ...
        loss, avr_rate_syn, avr_rate_asyn_no_add, avr_rate_asyn_add, wmmse_rate = sr_loss_all(data, out, train_K, Nt)
        loss.backward()
        ...
        optimizer.step()
    return total_loss / train_layouts, total_avr_rate_syn / train_layouts, total_avr_rate_asyn_no_add / train_layouts, total_avr_rate_asyn_add / train_layouts
...
def test():
    ...
    for data in test_loader:
        ...
        with torch.no_grad():
            ...
            loss, _, _, _, wmmse_rate = sr_loss_all(data, out, test_K, Nt)
...
for epoch in range(1, 3000):
    ...
    loss1, avr_rate_syn, avr_rate_asyn_no_add,  avr_rate_asyn_add= train()
    print('Start to test...')
    loss2, average_wmmse = test()
    print('SNR {:03d}dB, Epoch {:03d}, Train Loss: {:.4f}, avr_rate_syn: {:.4f}, avr_rate_asyn_no_add: {:.4f}, avr_rate_asyn_add: {:.4f}, Val Loss: {:.4f}, Average WMMSE Rate: {}'.format(
        SNR_dB, epoch, loss1, avr_rate_syn, avr_rate_asyn_no_add,  avr_rate_asyn_add, loss2, average_wmmse))
