...
def build_graph_MSCT_asyn(CSI, dist, delays, etas, norm_csi_real, norm_csi_imag, K, threshold):
    print('CSI:{}'.format(CSI))
    print('dist:{}'.format(dist))
...
def proc_data_asyn(HH, dists, delays, etas, norm_csi_real, norm_csi_imag, K):
    ...
    for i in range(n):
        # data = build_graph(HH[i,:,:,:],dists[i,:,:], norm_csi_real[i,:,:,:], norm_csi_imag[i,:,:,:], K,500)
        data = build_graph_MSCT_asyn(HH[i, :, :, :], dists[i, :, :], delays[i, :, :], etas[i, :, :], norm_csi_real[i, :, :, :],
...
class IGConv(MessagePassing):
    ...
    def update(self, aggr_out, x):
        ...
        comb_all = self.mlp2(tmp)
        print('update===comb_all:{}'.format(comb_all))
        comb = comb_all[:, 2:2 * Nt + 2] # w
        links_res = comb_all[:, 0:1] # alpha
        add_delta_delay = comb_all[:, 1:2] # tau_c
    ...
    for iter in range(H6.shape[0]):
        ...
        while t < t_max:
            ...
            for u_index in range(users):
                ...
                tmp_mmse = H_k[u_index,:,:] + v_k[u_index] * I_k
                tmp_zf = H_k[u_index,:,:]
                ...
                W_k_mmse[u_index, :] = submatrix_mmse / torch.sqrt(mode_square_sum_mmse)

                submatrix_zf = W_k_zf[u_index, :]
                mode_square_sum_zf = torch.sum(torch.abs(submatrix_zf) ** 2)
                W_k_zf[u_index, :] = submatrix_zf / torch.sqrt(mode_square_sum_zf)
            p_update = get_p_from_W(W_k, p3[iter, :, :, :])
            rx_all_power = torch.mul(H6[iter, :, :, :], p_update)
            rx_all_power = torch.sum(rx_all_power, axis=-1)
            t += 1
        ...
        for j in range(users):
            _, _, _, SINR_k = calculate_SINR(
                iter, rx_all_power, initial_delay, add_delta_delay, j, noise_powers[j], eta_enable, eta_add)
...
def sr_loss_all_test(data, p, K, N, epoch, imperfect_channel):
    ...
    syn_zf_rate = 0
    wmmse_exec_time = 0
    if only_GNN == 0 and (epoch == 1 or (epoch % 100 == 0)):
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        syn_zf_rate, asyn_zf_rate_noadd, asyn_zf_rate = sr_loss_all_test(data, out, train_K, Nt, epoch, 0)
